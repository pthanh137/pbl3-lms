...
class AnnouncementCreateSerializer(serializers.ModelSerializer):
    """Serializer for creating announcements (write-only fields)."""
    
    course_id = serializers.IntegerField(write_only=True)
    
    class Meta:
        model = Announcement
        fields = ['course_id', 'title', 'message']
    
    def validate_course_id(self, value):
        """Validate that course exists and user is the teacher."""
        ...
        if request and request.user:
            if course.teacher != request.user:
                raise serializers.ValidationError(
                    "You can only send announcements for your own courses."
                )
        
        return value
    
    def create(self, validated_data):
        """Create announcement with teacher from request."""
        ...
        # Create read status for all enrolled students
        from enrollments.models import Enrollment
        ...
        AnnouncementReadStatus.objects.bulk_create(read_statuses)
