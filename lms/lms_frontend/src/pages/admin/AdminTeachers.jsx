        ...
        {filteredTeachers.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
            ...
            <p className="text-slate-600">
              {searchQuery ? 'No teachers match your search' : 'No teachers registered yet'}
            </p>
          </div>
        ) : (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Teacher</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Email</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Headline</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Courses</th>
                    ...
                    <tr key={teacher.id} className="hover:bg-slate-50 transition">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          {teacher.avatar_url ? (
                            <img src={teacher.avatar_url} alt={teacher.full_name} className="w-10 h-10 rounded-full" />
                          ) : (
                            <div className="w-10 h-10 rounded-full bg-primary-500 flex items-center justify-center text-white font-semibold">
                              {teacher.full_name?.[0]?.toUpperCase() || teacher.email?.[0]?.toUpperCase() || 'T'}
                            </div>
                          )}
                          <div>
                            <div className="text-sm font-semibold text-slate-900">{teacher.full_name || 'No name'}</div>
                            <div className="text-xs text-slate-500">Joined {new Date(teacher.date_joined).toLocaleDateString()}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{teacher.email}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{teacher.headline || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{teacher.total_courses || 0}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{teacher.total_students || 0}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {teacher.is_approved ? (
